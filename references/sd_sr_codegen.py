...
class WeightLoaderManager:
    def __init__(self):
        ...
        self.bas_addr_cur = [1737, 792, 1152] # 代表3种parall_mode所用的buffer，这里需要考虑已经load的unet的权重
...
class QuantLoaderManager:
    def __init__(self):
        self.bas_addr_cur = 665 # 在过程中会不断累加上transnum，目前是scale和zp共用，假设是分开的两个buffer，需要考虑unet的参数
...
class OffsetLoaderManager:
    def __init__(self):
        self.bas_addr_cur = 0 
        self.offset_reg_idx = 0 # 0 or 1

    def offset_reg_idx_switch(self):
        self.offset_reg_idx = 1 if self.offset_reg_idx == 0 else 0
...
offsetloadermanager = OffsetLoaderManager()

def sd_inst(is_first = False, load_next = True):
    # 初始化
    weightloadermanager.bas_addr_cur = [0, 0, 0] 
    quantloadermanager.bas_addr_cur = 0
    datastorermanager.buffer_a_model = []
    datastorermanager.buffer_b_model = []
    offsetloadermanager.bas_addr_cur = 0
    ...
    # image 这里load的是fsrcnn的输入，这里不需要判断，因为只要有unet，后续必定有fsrcnn
    OffchipDataLoader.dispatch(
        transnum = 32*2,
        load_model = 1,
        src_buffer_idx = 0,
    ...
    for cal_idx in range(cal_total_num):
        ...
        DataStorer.dispatch(
            ...
            is_pixelshuffle = 0,
            pooling_out_mode = 1,
            pooling_out_new = 0 if cal_idx % 2 == 1 else 1,
            is_pooling = 1,
            reg_out_idx = datastorermanager.acc_reg_idx,
            acc_mode = 1,
            transfer_num = 1,
    ...
    OffchipDataStorer.dispatch(
        src_buffer = 'unet_output_reg',
        transnum = 18*2,
        base_addr = 0
    )


def sr_inst(load_next = True):
    # 初始化
    ...
    # 初始变量设定
    cal_total_num = 32 # 一次算出1行
    load_total_num = cal_total_num 
    dataloadermanager.line_buffer_load_total_num = load_total_num
    padding_num = 1
    dataloadermanager.bas_addr_cur = 144*4 # 存了原图 256*144
    datastorermanager.base_addrs_res_cur = 0
    datastorermanager.base_addr_pooling_cur = 0
    for cal_idx in range(cal_total_num):
        # DataLoader
        ...
        WeightLoader.dispatch(
            ...
            is_new = 0, # 开始新的时候覆盖
            transnum = 9,
            bas_addr = weightloadermanager.bas_addr_cur[2], 
            is_bilinear_bicubic = 0,
            offset_reg_idx = 0
        )
        weightloadermanager.line_buffer_idx_switch()
    ...
    # 收尾
    weightloadermanager.bas_addr_cur[2] += 9
    datastorermanager.buffer_a_model.append({"begin":0, "end":32*8, "info":"first_part_out", "valid":True})
    # datastorermanager.buffer_a_model[-1]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()
    ...
    # layer 1 32*32 mid_part_1_0 32,8,1
    layer_idx = 1
    quant_mode = 5
    QuantLoader.dispatch(
        quant_reg_load_idx = quantloadermanager.quant_config_idx,
    ...
    # 初始变量设定
    cal_total_num = 32//4 # 一次算出4行
    ic_load_num_per_cal = 8
    load_total_num = cal_total_num * ic_load_num_per_cal
    dataloadermanager.line_buffer_load_total_num = load_total_num * ic_load_num_per_cal
    padding_num = 1
    dataloadermanager.bas_addr_cur = 0
    datastorermanager.base_addrs_res_cur = 0
    datastorermanager.base_addr_pooling_cur = 32*2 # 为未池化的特征图预留空间
    for cal_idx in range(cal_total_num):
        for ic_load_num_per_cal_index in range(ic_load_num_per_cal):
            # DataLoader
            if cal_idx < padding_num:
                is_padding_row = 0 # 无需padding
            elif cal_idx > cal_total_num - 1 - padding_num:
                is_padding_row = 0
            else:
                is_padding_row = 0
            DataLoader.dispatch(
                ...
                line_buffer_idx = dataloadermanager.line_buffer_idx,
                src_buffer_idx = 'a',
                bas_addr = dataloadermanager.bas_addr_cur + ic_load_num_per_cal_index * 32
            )
            dataloadermanager.line_buffer_idx_switch()
            ...
            WeightLoader.dispatch(
                ...
                is_new = 0 if ic_load_num_per_cal_index == 0 else 1, # 开始新的时候覆盖
                transnum = 4,
                bas_addr = weightloadermanager.bas_addr_cur[0] + ic_load_num_per_cal_index * 4, 
                is_bilinear_bicubic = 0,
                offset_reg_idx = 0
            )
            weightloadermanager.line_buffer_idx_switch()
        ...
        datastorermanager.acc_reg_idx_switch()
        datastorermanager.base_addrs_res_cur += 4
        datastorermanager.base_addr_pooling_cur += 1

        weightloadermanager.acc_reg_idx_switch()
        dataloadermanager.bas_addr_cur += 4

    # 收尾
    weightloadermanager.bas_addr_cur[0] += 8*4
    datastorermanager.buffer_b_model.append({"begin":0, "end":32*2, "info":"mid_padr_1_0_out", "valid":True})
    datastorermanager.buffer_b_model.append({"begin":32*2, "end":32*2 + 8, "info":"mid_padr_1_0_pool_out", "valid":True})
    datastorermanager.buffer_a_model[-1]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()

    # layer 2、3 32*32 mid_part_1_1 dconv 8,8,3
    # offset生成
    layer_idx = 2 
    quant_mode = 2
    QuantLoader.dispatch(
    ...
    # 初始变量设定
    cal_total_num = 1 # 一次算出所有offset
    ky_load_num_per_cal = 3
    load_total_num = cal_total_num * ky_load_num_per_cal
    dataloadermanager.line_buffer_load_total_num = load_total_num * ky_load_num_per_cal
    padding_num = 1
    dataloadermanager.bas_addr_cur = 32*2
    datastorermanager.base_addrs_res_cur = 0
    datastorermanager.base_addr_pooling_cur = 0
    for cal_idx in range(cal_total_num):
        for ky_load_num_per_cal_index in range(ky_load_num_per_cal):
            ...
            WeightLoader.dispatch(
                ...
                is_new = 0 if ky_load_num_per_cal_index == 0 else 1, # 开始新的时候覆盖
                transnum = 24,
                bas_addr = weightloadermanager.bas_addr_cur[1] + ky_load_num_per_cal_index * 24, 
                is_bilinear_bicubic = 0,
                offset_reg_idx = 0
            )
            weightloadermanager.line_buffer_idx_switch()
    ...
    # 收尾
    weightloadermanager.bas_addr_cur[1] += 24*3
    # datastorermanager.buffer_b_model.append({"begin":0, "end":32*2, "info":"mid_padr_1_0_out", "valid":True})
    # datastorermanager.buffer_b_model.append({"begin":32*2, "end":32*2 + 8, "info":"mid_padr_1_0_pool_out", "valid":True})
    datastorermanager.buffer_b_model[-1]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    ...
    # 初始变量设定
    cal_total_num = 32//4 # 一次算出4行
    ky_load_num_per_cal = 3 # 注意这里是有ky遍历的
    ic_load_num_per_cal = 2
    load_total_num = cal_total_num * ky_load_num_per_cal * ic_load_num_per_cal
    dataloadermanager.line_buffer_load_total_num = load_total_num * ky_load_num_per_cal * ic_load_num_per_cal
    padding_num = 1
    dataloadermanager.bas_addr_cur = 0
    datastorermanager.base_addrs_res_cur = 0
    datastorermanager.base_addr_pooling_cur = 32*2
    for cal_idx in range(cal_total_num):
        for ky_load_num_per_cal_index in range(ky_load_num_per_cal):
            # load offset
            OffsetLoader.dispatch(
                offset_reg_idx = offsetloadermanager.offset_reg_idx, 
                bas_addr = cal_idx * ky_load_num_per_cal + ky_load_num_per_cal_index
            )
            for ic_load_num_per_cal_index in range(ic_load_num_per_cal):
                # DataLoader
                if cal_idx < padding_num and ky_load_num_per_cal_index == 0:
                    is_padding_row = 4 
                elif cal_idx < padding_num and ky_load_num_per_cal_index == 1:
                    is_padding_row = 1 
                elif cal_idx > cal_total_num - 1 - padding_num and ky_load_num_per_cal_index == 1:
                    is_padding_row = 5
                elif cal_idx > cal_total_num - 1 - padding_num and ky_load_num_per_cal_index == 2:
                    is_padding_row = 6
                else:
                    is_padding_row = 0
                DataLoader.dispatch(
                    ...
                    line_buffer_idx = dataloadermanager.line_buffer_idx,
                    src_buffer_idx = 'b',
                    bas_addr = dataloadermanager.bas_addr_cur + ic_load_num_per_cal_index * 32 + (ky_load_num_per_cal_index if cal_idx > 0 else 0) 
                )
                dataloadermanager.line_buffer_idx_switch()
                ...
                weightloadermanager.line_buffer_idx_switch()
            # 切换offset_reg_idx
            offsetloadermanager.offset_reg_idx_switch()
        ...
        datastorermanager.acc_reg_idx_switch()
        datastorermanager.base_addrs_res_cur += 4 # 一次算出4行
        datastorermanager.base_addr_pooling_cur += 1

        weightloadermanager.acc_reg_idx_switch()
        dataloadermanager.bas_addr_cur += 2 if cal_idx < padding_num else 4 # 因为算padding了两行，所以第一次只能加2

    # 收尾
    weightloadermanager.bas_addr_cur[0] += 12*6
    datastorermanager.buffer_a_model.append({"begin":0, "end":32*2, "info":"mid_part_1_1_out", "valid":True})
    datastorermanager.buffer_a_model.append({"begin":32*2, "end":32*2 + 8, "info":"mid_part_1_1_pool_out", "valid":True})
    datastorermanager.buffer_b_model[-2]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()

    # layer 4、5 32*32 mid_part_1_2 dconv 8,8,3
    # offset生成
    layer_idx = 4 
    quant_mode = 2
    QuantLoader.dispatch(
    ...
    # 收尾
    weightloadermanager.bas_addr_cur[0] += 12*6
    datastorermanager.buffer_b_model.append({"begin":0, "end":32*2, "info":"mid_part_1_2_out", "valid":True})
    datastorermanager.buffer_b_model.append({"begin":32*2, "end":32*2 + 8, "info":"mid_part_1_2_pool_out", "valid":True})
    datastorermanager.buffer_a_model[-2]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()

    # layer 6、7 32*32 mid_part_2_0 dconv 8,8,3
    # offset生成
    layer_idx = 6 
    quant_mode = 2
    QuantLoader.dispatch(
    ...
    # 收尾
    weightloadermanager.bas_addr_cur[0] += 12*6
    datastorermanager.buffer_a_model.append({"begin":0, "end":32*2, "info":"mid_part_2_0_out", "valid":True})
    datastorermanager.buffer_a_model.append({"begin":32*2, "end":32*2 + 8, "info":"mid_part_2_0_pool_out", "valid":True})
    datastorermanager.buffer_b_model[-2]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()

    # layer 8、9 32*32 mid_part_2_1 dconv 8,8,3
    # offset生成
    layer_idx = 8 
    quant_mode = 2
    QuantLoader.dispatch(
    ...
    # 收尾
    weightloadermanager.bas_addr_cur[0] += 12*6
    datastorermanager.buffer_b_model.append({"begin":0, "end":32*2, "info":"mid_part_2_1_out", "valid":True})
    datastorermanager.buffer_a_model[-2]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()

    # layer 10 32*32 mid_part_2_2 8,32,1
    layer_idx = 10
    quant_mode = 3
    QuantLoader.dispatch(
        quant_reg_load_idx = quantloadermanager.quant_config_idx,
    ...
    # 收尾
    weightloadermanager.bas_addr_cur[2] += 8
    datastorermanager.buffer_a_model.append({"begin":0, "end":32*8, "info":"mid_part_2_2_out", "valid":True})
    datastorermanager.buffer_b_model[-1]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()
    
    # layer 11 32*32 last_part 32,4,3
    layer_idx = 11
    quant_mode = 5
    QuantLoader.dispatch(
        quant_reg_load_idx = quantloadermanager.quant_config_idx,
    ...
    for cal_idx in range(cal_total_num):
        for ky_load_num_per_cal_index in range(ky_load_num_per_cal):
            for ic_load_num_per_cal_index in range(ic_load_num_per_cal):
                ...
                DataLoader.dispatch(
                    ...
                    line_buffer_idx = dataloadermanager.line_buffer_idx,
                    src_buffer_idx = 'a',
                    bas_addr = dataloadermanager.bas_addr_cur + ic_load_num_per_cal_index * 32 + (max(ky_load_num_per_cal_index - 1, 0) if cal_idx < padding_num else ky_load_num_per_cal_index)
                )
                dataloadermanager.line_buffer_idx_switch()
                ...
                WeightLoader.dispatch(
                    acc_reg_comp_idx = weightloadermanager.acc_reg_idx,
                    kernal_size = 0,
                    line_buffer_row_shift = 2,
                    line_buffer_idx = weightloadermanager.line_buffer_idx,
                    is_padding_col = 3,
                    weight_parall_mode = 0,
                    is_new = 0 if ky_load_num_per_cal_index == 0 and ic_load_num_per_cal_index == 0 else 1, # 开始新的时候覆盖
                    transnum = 6,
                    bas_addr = weightloadermanager.bas_addr_cur[0] + 6*(ky_load_num_per_cal_index * ic_load_num_per_cal + ic_load_num_per_cal_index), 
                    is_bilinear_bicubic = 0,
                    offset_reg_idx = 0
                )
                weightloadermanager.line_buffer_idx_switch()
        ...
        DataStorer.dispatch(
            ...
            is_mask = 0,
            is_new = 0,
            dest_buffer_idx = 'fsrcnn_output_buffer' # 存到fsrcnn_output_buffer
        )
        datastorermanager.acc_reg_idx_switch()
        datastorermanager.base_addrs_res_cur += 128 # fsrcnn_output_buffer一行存4个数

        weightloadermanager.acc_reg_idx_switch()
        dataloadermanager.bas_addr_cur += 3 if cal_idx < padding_num else 4

    # 收尾
    weightloadermanager.bas_addr_cur[0] += 6*24
    # datastorermanager.buffer_a_model.append({"begin":0, "end":32*8, "info":"mid_part_2_2_out", "valid":True})
    datastorermanager.buffer_a_model[-1]["valid"] = False
    quantloadermanager.quant_config_idx_switch()
    datastorermanager.quant_config_idx_switch()
    ...
    # off store
    OffchipDataStorer.dispatch(
        src_buffer = 'fsrcnn_output_buffer',
        transnum = 1024,
        base_addr = 0
    )

# Bicubic
def bicubic_inst(off_store = False, load_next = True):
    layer_idx = 0 # 设置为第0层 方便依赖判断
    quant_mode = 5
    QuantLoader.dispatch(
        quant_reg_load_idx = quantloadermanager.quant_config_idx,
        quant_mode = quant_mode,
        layer_idx = layer_idx,
        transnum = 4,
        bas_addr = 837 # 这里直接取用bicubic的量化参数地址
    )

    # 初始变量设定
    cal_total_num = 33 # 需要遍历33次
    load_total_num = cal_total_num 
    dataloadermanager.line_buffer_load_total_num = load_total_num 
    dataloadermanager.bas_addr_cur = 144*4
    datastorermanager.base_addrs_res_cur = 0
    ...
    for cal_idx in range(cal_total_num):
        ...
        if cal_idx == 0:
            ...
            is_padding_row = 1 
        elif cal_idx == cal_total_num - 2:
            is_padding_row = 5
        elif cal_idx == cal_total_num - 1:
            is_padding_row = 6
        else:
            is_padding_row = 0
        DataLoader.dispatch(
        ...
        WeightLoader.dispatch(
            ...
            is_new = 0, # 开始新的时候覆盖
            transnum = 16,
            bas_addr = 2201, # 这里直接取用bicubic的权重地址 1737 + 464
        ...
        datastorermanager.acc_reg_idx_switch()
        datastorermanager.base_addrs_res_cur += 16 if cal_idx == 0 else 32 # fsrcnn_output_buffer一行存4个数

        weightloadermanager.acc_reg_idx_switch()
        dataloadermanager.bas_addr_cur += 2 # offchip中一个数据存两行
...
def sd_sr_inst(image_num = 1, model_list = []):
    image_block_num = 18*32
    for image_num_idx in range(image_num):
        is_first = True if image_num_idx == 0 else False
        load_next_sd = True if image_num_idx < image_num - 1 else False
        sd_inst(is_first, load_next_sd)
        
        # 需要超分的图块列表
        sub_model_list = model_list[image_num_idx]
        for image_block_num_idx in range(image_block_num):
            load_next_sr = True if image_block_num_idx < image_block_num - 1 else False
            if image_block_num_idx in sub_model_list:
                bicubic_inst(off_store = False, load_next = load_next_sr)
                sr_inst(load_next = load_next_sr)
            else:
                bicubic_inst(off_store = True, load_next = load_next_sr)
...
if __name__ == "__main__":
    model_list = [
        [108, 109, 110, 111, 140, 141, 142, 143, 171, 172, 173, 174, 175, 202, 203, 204, 205, 206, 234, 235, 236, 237, 238, 239, 266, 267, 268, 269, 270, 271, 298, 299, 300, 301, 302, 303, 304, 330, 331, 332, 333, 334, 335, 336, 362, 363, 364, 365, 366, 367, 368, 394, 395, 396, 397, 398, 399, 400, 426, 427, 428, 429, 430, 431, 432, 458, 459, 460, 461, 462, 463, 464, 491, 492, 493, 494, 495, 496, 523, 524, 525, 526, 527, 528, 555, 556, 557, 558, 559],
        [371, 401, 402, 403, 433, 434, 435, 464, 465, 496, 497, 498, 529, 530, 561, 562, 563],
        [173, 174, 181, 182, 183, 205, 206, 212, 213, 214, 215, 237, 238, 239, 240, 243, 244, 245, 246, 265, 266, 268, 269, 270, 271, 272, 273, 274, 275, 276, 277, 297, 298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 331, 332, 333, 334, 335, 336, 337, 338, 339, 340, 341, 363, 364, 365, 366, 367, 368, 369, 370, 371, 372, 373, 395, 396, 397, 398, 399, 400, 401, 402, 403, 404, 428, 429, 430, 431, 432, 433, 434, 435, 436, 461, 462, 463, 464, 465, 466, 467, 495, 496, 497]
    ]
    
    # sd_sr_inst(len(model_list), model_list)
    conv_test_inst()
        ...
        if code_list[i]['op_code'] == 'OffchipDataLoader':
            ...
            for d in range(i-1,-1,-1):
                if code_list[d]['op_code'] == 'DataStorer':
                    if code_list[d]["reg_out_idx"] == code_list[i]["acc_reg_comp_idx"]:
                        code_list[i]['dependency'].append(d)
                        break
                if code_list[d]['op_code'] == 'WeightLoader': # 防止冗余依赖，利用了weightloader总是依赖上一条weightloader
            ...
            for d in range(i-1,-1,-1):
                if code_list[d]['op_code'] == 'WeightLoader': 
                    code_list[i]['dependency'].append(d)
                    break
            if code_list[i]['is_bilinear_bicubic'] == 1: # 即需要使用offset
                # 这里利用了weightloader总是依赖上一条weightloader
                if code_list[code_list[i]['dependency'][-1]]['is_bilinear_bicubic'] == 0 or (code_list[code_list[i]['dependency'][-1]]['is_bilinear_bicubic'] == 1 and code_list[code_list[i]['dependency'][-1]]['offset_reg_idx'] != code_list[i]['offset_reg_idx']):
                    for d in range(i-1,-1,-1):
            ...
            if code_list[i]['layer_idx'] == 0:
                ...
                for d in range(i-1,-1,-1):
                    if code_list[d]['op_code'] == 'DataLoader' and dataloader_count < 2:
                        if code_list[d]['layer_idx'] == 0:
                            dataloader_count += 1
                    if dataloader_count == 2 or code_list[d]['op_code'] == 'OffchipDataStorer': # 这里用OffchipDataStorer是为了判断是否跨了图像块
                        break
                # 只有前两个需要处理
                if dataloader_count < 2:
                    if code_list[i]['bas_addr'] < 144*4: # unet的第一层
                        for d in range(i-1,-1,-1):
                            if code_list[d]['op_code'] == 'OffchipDataLoader':
                                if code_list[d]['src_buffer_idx'] == 0 and code_list[d]['load_model'] == 0:
                                    code_list[i]['dependency'].append(d)
                                    break
                    if code_list[i]['bas_addr'] >= 144*4: # fsrcnn的第一层
                        for d in range(i-1,-1,-1):
                            if code_list[d]['op_code'] == 'OffchipDataLoader':
                                if code_list[d]['src_buffer_idx'] == 0 and code_list[d]['load_model'] == 1:
                                    code_list[i]['dependency'].append(d)
                                    break
            else: # 非第0层
                # 处理 DataStorer 依赖
            ...
            for d in range(i-1,-1,-1): 
                if code_list[d]['op_code'] == 'OffchipDataLoader':
                    if code_list[d]["src_buffer_idx"] == 2:  # 等候片外quant数据加载完
                        code_list[i]['dependency'].append(d)
                        break
                if code_list[d]['op_code'] == 'QuantLoader': # 只有前两个QuantLoader要等，后续自动满足
                    quantloader_count += 1
            ...
            for d in range(i-1,-1,-1):
                if code_list[d]['op_code'] == 'QuantLoader':
                    if code_list[d]["quant_reg_load_idx"] == code_list[i]["quant_config_idx"]: 
                        code_list[i]['dependency'].append(d)
                        break
                if code_list[d]['op_code'] == 'DataStorer': # 利用DataStorer总是依赖上一个DataStorer
            ...
            for d in range(i-1,-1,-1):
                if code_list[d]['op_code'] == 'DataStorer':
                    code_list[i]['dependency'].append(d)
                    break
            if code_list[i]['dest_buffer_idx'] == 'fsrcnn_output_buffer' or code_list[i]['dest_buffer_idx'] == 'unet_output_reg': # 只针对存在片外交互buffer的dataloader指令
                for d in range(i-1,-1,-1):
            ...
            for d in range(i-1,-1,-1):
                if code_list[d]['op_code'] == 'WeightLoader':
                    if code_list[d]["offset_reg_idx"] == code_list[i]["offset_reg_idx"] and code_list[d]['is_bilinear_bicubic'] == 1: 
                        code_list[i]['dependency'].append(d)
                        break
                    if code_list[d]['is_bilinear_bicubic'] == 0: # 避免找寻到上一个图块的weightloader
                        break
            # 对于 DataStorer 的依赖只需要前两条即可，其他的自然满足
    ...
    # 逐opcode找最大gap
    opcode_dict = {"OffchipDataLoader":0,
                    "DataLoader":1,
                    "WeightLoader":2,
                    "OffsetLoader":3,
                    "QuantLoader":4,
                    "DataStorer":5,
                    "OffchipDataStorer":6}
    max_gap_list = [0,0,0,0,0,0,0]
    max_gap_idx_list = [0,0,0,0,0,0,0]
    for i in range(len(code_list)): 
        for t in code_list[i]['dependency']:
            if i-t > max_gap_list[opcode_dict[code_list[i]['op_code']]]:
                max_gap_list[opcode_dict[code_list[i]['op_code']]] = i-t
                max_gap_idx_list[opcode_dict[code_list[i]['op_code']]] = i
    print(max_gap_list)
    print(max_gap_idx_list)
    
    # 处理dest与src赋值  
    idle_reg_id = list(range(1, 16))[::-1]
    init_len = len(idle_reg_id)
    reg_used_count_max = 0 # 结果是最多用到 dest = 
    ...
    for i in range(len(code_list)):
        ...
        for occ_pair in reversed(occupy_list): # 通过reversed和append才能使得尽可能从小的编号开始分配
            required = False
            for k in range(i + 1, min(len(code_list), i + max_gap + 10)):
    ...
    #         with open(f"code/sr/debug/layer{layer_idx_mark}/pseudo_code_%s_with_dependency_and_reg_num.txt" % date.strftime("%m%d%H%M"), "w") as f11:
    ...
    date = datetime.datetime.now()
    with open("code/conv_test_data/pseudo_code_%s_with_dependency_and_reg_num.txt" % date.strftime("%m%d%H%M"), "w") as f11:
        f11.writelines(list(map(lambda a: str(a) + '\n', code_list)))  
    ...
    #     if code_list[i]['op_code'] == 'QuantLoader' and code_list[i]['layer_idx'] == layer_idx_mark + 1:
    #         date = datetime.datetime.now()
    #         with open(f"code/sr/debug/layer{layer_idx_mark}/pseudo_code_%s.txt" % date.strftime("%m%d%H%M"), "w") as f11:
    #             f11.writelines(list(map(lambda a: str(a) + '\n', code_list_print)))
    #         len_mark += len(code_list_print)
    #         code_list_print = []
    #         code_list_print.append(code_list[i])
    ...
    # code_list = Inst.code_list
    # date = datetime.datetime.now()
    # with open("code/sr/pseudo_code_%s.txt" % date.strftime("%m%d%H%M"), "w") as f11:
    #     f11.writelines(list(map(lambda a: str(a) + '\n', code_list)))
